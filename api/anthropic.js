...
// If UPSTASH_REDIS_REST_URL and UPSTASH_REDIS_REST_TOKEN are set, this uses Upstash REST API
// to enforce a shared rate limit counter across instances. Otherwise it falls back to
// an in-memory best-effort rate limiter (works per-warm-instance only).
...
async function upstashIncrWithExpire(key) {
  // Use Upstash REST API: POST /incr/{key} then POST /expire/{key}/{seconds} when newly created
  ...
  // Upstash returns {"result":NUMBER} for some endpoints or a plain number for others
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Rate limiting (Upstash if configured, else in-memory best-effort)
    ...
    if (USE_UPSTASH) {
      try {
        ...
        console.error('Upstash error, falling back to in-memory rate limit:', e && e.message ? e.message : e);
